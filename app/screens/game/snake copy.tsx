        ...
        <GestureHandlerRootView style={{ flex: 1 }}>
          <PanGestureHandler onGestureEvent={handleGesture}>
            <View style={styles.gameArea}>
              {/* Snake rendering */}
              {snake.map((segment, i) =>
                i === 0 ? ( // Snake head with proper rotation
                  <View
                    key={i}
                    style={{
                      position: "absolute",
                      left: segment.x * BLOCK_SIZE - 5, // Centered adjustment
                      top: segment.y * BLOCK_SIZE - 5, // Centered adjustment
                      width: BLOCK_SIZE + 25,
              ...
              {/* Food items with better spacing */}
              {food.map((f, i) => (
                <View
                  key={i}
                  style={[
                    styles.food,
                    {
                      ...
                      top: Math.max(
                        ...
                        Math.min(
                          f.y * BLOCK_SIZE,
                          GAME_HEIGHT - FOOD_BLOCK_SIZE - 15
                        )
                      ), // Boundary check
                      width: FOOD_BLOCK_SIZE + 20,
                      height: FOOD_BLOCK_SIZE + 20,
                      borderRadius: 10, // More rounded corners
                      backgroundColor: `hsl(${(i * 60) % 360}, 80%, 50%)`, // Color variation
                    },
                  ]}
                >
                  <Text
